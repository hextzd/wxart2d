...
kbNode::kbNode( kbBool_Engine* GC ) : kbLPoint( 0, 0 )
{
    _GC = GC;
...
kbNode::kbNode( B_INT const X, B_INT const Y, kbBool_Engine* GC ) : kbLPoint( X, Y )
{
    _GC = GC;
    _linklist = new DL_List<void*>();
}
...
kbNode::kbNode( kbLPoint* const a_point, kbBool_Engine* GC ) : kbLPoint( a_point )
{
    _GC = GC;
    _linklist = new DL_List<void*>();
}
...
kbNode::kbNode( kbNode * const other, kbBool_Engine* GC )
{
    _GC = GC;
    _x = other->_x;
    _y = other->_y;
...
kbLink* kbNode::GetHoleFlaggedLink( bool checkbin )
{
    kbLink * Result = NULL, *link;
