...
kbGraphList::kbGraphList( kbBool_Engine* GC )
{
    _GC = GC;
}
...
void kbGraphList::Prepare( kbGraph* total )
{
    ...
    Simplify( ( double ) _GC->GetInternalMarge() );
    ...
    if ( ! _GC->GetOrientationEntryMode() )
    {
        ...
        while ( !_LI.hitroot() )
        {
            if ( _LI.item()->GetFirstLink()->GetHole() )
                _LI.item()->MakeCounterClockWise();
            else
    ...
    // in out calculation per graph ( at this stage original polygons )
    // is based on the graph number.
    Renumber();
...
{
    // list contains polygon contours clockwise and holes counter clockwise.
    // This is orientation. GroupA holes are seen as holes in groupA contours.
    // therefore all the same number.
    TDLI<kbGraph> _LI = TDLI<kbGraph>( this );
    _LI.tohead();
    while ( !_LI.hitroot() )
    {
    ...
    if ( _GC->GetInternalCorrectionFactor() ) //not zero
    {
        ...
        for( int i = 0; i < todo ; i++ )
        {
            ...
            kbGraphList *_correct = new kbGraphList( _GC );
            {
                ...
                while (  !_LI.hitroot()  && _LI.item()->GetFirstLink()->GetHole() )
                {
                    graph->TakeOver( _LI.item() );
                    delete _LI.item();
                    _LI.remove();
                    i++;
                }
                //graph->MakeClockWise();
                graph->Correction( _correct, _GC->GetInternalCorrectionFactor() );
                delete graph;
...
void kbGraphList::Boolean( BOOL_OP operation, int intersectionRunsMax )
{
    ...
    while ( intersectionruns <= intersectionRunsMax )
    {
        ...
        {
            if ( _GC->GetDebug() )
                _prepared->WriteGraphKEY( _GC , "atKboolErrorTry.key" );

            intersectionruns++;
            if ( intersectionruns == intersectionRunsMax )
            {
                if ( _GC->GetDebug() )
                    _prepared->WriteGraphKEY( _GC, "atKboolErrorMaxTry.key" );
        ...
        catch( ... )
        {
            ...
            {
                if ( _GC->GetDebug() )
                    _prepared->WriteGraphKEY( _GC, "atKboolErrorUnKnown.key" );
...
void kbGraphList::WriteGraphsKEY( kbBool_Engine* GC )
{
