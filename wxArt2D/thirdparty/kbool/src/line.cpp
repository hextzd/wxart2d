...
//
kbLine::kbLine( kbBool_Engine* GC )
{
    m_GC = GC;
    m_AA = 0.0;
    m_BB = 0.0;
...
//
kbLine::kbLine( kbLink *a_link, kbBool_Engine* GC )
{
    m_GC = GC;
    // a_link must exist
    assert( a_link );
...
int kbLine::Intersect( kbLine * lijn, double Marge )
{
    ...
    case 2: case 6: m_link->AddCrossing( ep );
            Number_of_Crossings = 1;
            break;
    case 3: case 5: m_link->AddCrossing( bp );
            Number_of_Crossings = 1;
            break;
        case 4:     m_link->AddCrossing( bp );
            m_link->AddCrossing( ep );
...
case 1: case 5: case 6:
        {
            ...
            switch ( Take_Action2 )
            {
                ...
                {   // begin of scope to calculate the intersection
                    ...
                    m_link->AddLineCrossing( ( B_INT )X, ( B_INT )Y, lijn->m_link );
                }   // end of scope to calculate the intersection
                Number_of_Crossings++;
                break;
                case 2: lijn->m_link->AddCrossing( ep );
                    Number_of_Crossings++;
                    break;
                case 3: lijn->m_link->AddCrossing( bp );
                    Number_of_Crossings++;
                    break;
                case 4: lijn->m_link->AddCrossing( bp );
                    lijn->m_link->AddCrossing( ep );
                    Number_of_Crossings = 2;
                    break;
            }
        }
