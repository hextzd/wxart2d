...
class A2DKBOOLDLLEXP kbBool_Engine_Error
{
public:
    kbBool_Engine_Error( string message, string header = 0, int degree = 9, int fatal = 0 );
    kbBool_Engine_Error( const kbBool_Engine_Error& a );
    ~kbBool_Engine_Error();
...
*/
class A2DKBOOLDLLEXP kbBool_Engine
{
...
// since they are scaled with m_DGRID
    ...
    */
    ...
    bool GetDebug() { return m_doDebug; }
    ...
    */
    ...
    /* Add a polygon and mark it as a hole.
       The polygon is added, but a flag is set to identify it as a hole.
       Those polygons will be made counterclockwise, before performing the poolean operation.
       Normal polygons will be made clockwise.
       Hole polygons should be part of an outer contour.
    */
    bool StartHolePolygonAdd( GroupType A_or_B );

    //! see StartPolygonAdd
    bool AddPoint( double x, double y, bool arcPiece = false );

    //! see StartPolygonAdd
    bool EndPolygonAdd( bool arcPiece = false );
...
private:
    ...
    //1 true is a hole polygons is being added, else false.
    bool m_holePolygon;
