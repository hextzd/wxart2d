...
void DrawingView::OnDraw( wxDC* dc )
{
    ...
    wxList::compatibility_iterator iSegment =
        ( ( DrawingDocument* )GetDocument() )->GetDoodleSegments().GetFirst();
    while ( iSegment )
    {
...
BEGIN_EVENT_TABLE( EditorFrame, a2dDocumentFrame )
    ...
    EVT_CHANGED_DOCUMENT( EditorFrame::OnChangedDocument )
END_EVENT_TABLE()
...
EditorFrame::~EditorFrame()
{
    ...
    //after activating the frame, get events from commandprocessor of document, remove it here
    if ( m_view && m_view->GetDocument() )
    {
        m_view->GetDocument()->DisconnectEvent( wxEVT_MENUSTRINGS, this );
        m_view->GetDocument()->DisconnectEvent( wxEVT_DO, this );
        m_view->GetDocument()->DisconnectEvent( wxEVT_UNDO, this );
    }
    a2dDocviewGlobals->GetDocviewCommandProcessor()->DisconnectEvent( wxEVT_NEW_DOCUMENT, this );
...
void EditorFrame::OnChangedDocument( a2dCommandEvent& event )
{
...
{
    if ( viewevent.GetEventObject() == m_view )
    {
        if ( viewevent.GetActive() )
        {
            ...
            //after activating the frame, get events from commandprocessor of document
            if ( doc && doc->GetCommandProcessor() )
            {
                doc->ConnectEvent( wxEVT_MENUSTRINGS, this );
        ...
        else
        {
            ...
            if ( doc && doc->GetCommandProcessor() )
            {
                doc->DisconnectEvent( wxEVT_NULL, this );
                doc->DisconnectEvent( wxEVT_MENUSTRINGS, this );
                doc->DisconnectEvent( wxEVT_DO, this );
                doc->DisconnectEvent( wxEVT_UNDO, this );
                a2dDocviewGlobals->GetDocviewCommandProcessor()->DisconnectEvent( wxEVT_NEW_DOCUMENT, this );
                a2dDocviewGlobals->GetDocviewCommandProcessor()->DisconnectEvent( wxEVT_CHANGED_DOCUMENT, this );
            }
        }
